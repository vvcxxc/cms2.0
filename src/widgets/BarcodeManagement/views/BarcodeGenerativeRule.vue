  ...
  <div class="bar-content">
    <div class="bar-header">
      <IconButton
        ...
        type="primary"
        @click="onAdd"
        v-permission="'generate-management-add'"
      >
        新增
      </IconButton>
      <IconButton
        icon="edit"
        @click="onEdit"
        v-permission="'generate-management-edit'"
      >
        编辑
      </IconButton>
      <IconButton
        v-permission="'generate-management-delete'"
        icon="delete"
        @click="onDelete"
        >删除</IconButton
      >
      <IconButton
        v-permission="'generate-management-view'"
        icon="view"
        @click="onView"
        >查看</IconButton
      >
...
import { getBarCodeList, deleteData, message } from '../action'
// import Table from 'components/Table/index.vue'
import BarcodeGenerativeRuleDialog from '../components/BarcodeGenerativeRuleDialog.vue'
import { columns, Row, current } from '../state'
...
const checkedList = ref<Row[]>([])
const isView = ref(false)

const setCurrent = (row: any) => {
  current.value = row ? cloneDeep(row) : row
}
...
const onDelete = (row: any) => {
  ...
  ConfirmBox('条码删除后不可恢复，是否确认删除？').then(async () => {
    ...
    await deleteData(ids as string[])
...
// const onSort = () => {
//   params.Sorting = 'LastModificationTime'
//   getList()
// }

// const onPageChange = (page: number) => {
//   params.SkipCount = (page - 1) * MaxResultCount
//   getList()
// }
